...
### 칩 시스템
- 시작 자산: **100,000칩** (로그인(가입) 시 지급)
- **라운드 보너스 없음.** 시작 칩 100,000으로 대회 끝까지 진행.
...
| 조별리그 진출팀 | `advance` | `rarity_share` | true | 조별리그 종료 후 (32팀 진출) |
...
### 희소성 분배 (`rarity_share`) — 진출팀 (희소성 보너스)
진출팀 카테고리에 사용. **팀당 고정 칩(1,000)으로 정확히 32팀 선택**, 확정 전까지 수정 가능.

```
전체 풀 = 모두가 건 칩 합
각 진출(정답)팀 몫 = 전체 풀 / '누군가 고른 진출팀 수'
내 배당금 = Σ(내가 고른 진출팀들의 [그 팀 몫 / 그 팀을 고른 사람 수])
```

**예시**: 풀 6,000. 진출팀 3개(한국·브라질·멕시코), 각 몫 2,000.
- 한국(나만) → 2,000, 브라질(셋) → 666씩, 멕시코(B만) → 2,000
- → **남이 적게 고른 진출팀을 맞힐수록 더 큰 이익.** 탈락팀에 건 칩은 손실(보너스 재원).
- 진출팀을 아무도 못 고르면 전원 환불.
